...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        int chunks = (a.getNRows() + size - 1) / size;

        int start_row = myrank * chunks;
        int end_row = (myrank + 1) * chunks;
        if (end_row > a.getNRows()) end_row = a.getNRows();
        ...
        if (myrank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                int rankStartRow = i * chunks;
                int rankEndRow = (i + 1) * chunks;
                if (rankEndRow > a.getNRows()) rankEndRow = a.getNRows();
            ...
            mpi.MPI_Send(c.getValues(), start_row * c.getNCols(),
                    (end_row - start_row) * c.getNCols(), 0, myrank,
                    mpi.MPI_COMM_WORLD);
        }
    }
}
