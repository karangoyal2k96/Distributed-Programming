...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        JavaPairRDD<Integer,Double> new_ranks=sites.join(ranks).flatMapToPair(
                kv ->{
                    Integer Website_Id=kv._1;
                    Tuple2<Website,Double> Info=kv._2;

                    Website website= Info._1;
                    Double rank= Info._2;

                    Double part=rank/website.getNEdges();
                    LinkedList<Tuple2<Integer,Double>> temp= new LinkedList<Tuple2<Integer,Double> >();

                    Iterator<Integer> it=website.edgeIterator();

                    while(((Iterator) it).hasNext()){

                       ((LinkedList) temp).add(new Tuple2<Integer,Double>(it.next(),part));

                    }
                    return temp;
                }
        );
                return  new_ranks.reduceByKey((Double r1,Double r2)->r1+r2).mapValues(v->0.15+0.85*v);
